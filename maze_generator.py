...
def generate_maze(size: int, player_pos: tuple) -> list:
    maze = []
    for i in range(size):
        maze.append([])
        for j in range(size):
...
def find_shortest_path(maze: list, player_pos) -> list:
    start = player_pos
    end = (0, 0)
    ...
    while queue:
        ...
        for next in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
            ...
            if (
                0 <= next_step[0] < len(maze)
                and 0 <= next_step[1] < len(maze)
                and maze[next_step[0]][next_step[1]] not in [1, 4]
...
def draw_path(maze: list, path: list, screen, color: tuple, WIDTH: int, HEIGHT: int):
    for step in path:
        pg.draw.rect(
            screen,
            color,
            pg.Rect(
                ((WIDTH // len(maze)) * step[1], HEIGHT // len(maze) * step[0]),
                (WIDTH // len(maze) - 1, HEIGHT // len(maze) - 1),
            ),
        )
        pg.display.flip()
        time.sleep(1 / (len(path)))
    time.sleep(1)
