...
def render_end_of_game(text: str):
    lost_text = Text(text, (WIDTH / 2, HEIGHT / 2))
    screen.fill("white")
    lost_text.render_to_screen(screen)


def main(maze_size: int) -> int:
    # maze_size = 200
    steps = 0
    player = Player((randint(2, maze_size - 2), randint(2, maze_size - 2)))
    maze = []
    path = []
    while steps < maze_size * 1.5:
        maze = generate_maze(maze_size, player.get_current_position())
    ...
    quick_restart_text = Text("'r' - quick restart", (0, HEIGHT / 10), center=False)
    restart_text = Text("'p' - restart", (0, HEIGHT / 10 + 24), center=False)
    restart_btn = Button(
        "Restart", (WIDTH / 2, HEIGHT * 2 // 3), (WIDTH / 4, HEIGHT / 8)
    )
    quit_btn = Button("Quit", (WIDTH - 30, 15), (60, 30))
